$(function(){
    ...
    $('a[ds_type="batchbutton"]').click(function () {
        ...
        if ($('.checkitem:checked').length == 0) {    //没有选择
            layer.msg('Por favor, seleccione el registro que desea manipular');
            return false;
        }
...
function check_number(v)
{
    ...
    if(isNaN(v))
    {
        alert("Debe ser un número");//lang.only_number
        return false;
    }
    if(v.indexOf('-') > -1)
    {
...
function check_required(v)
{
    if(v == '')
    {
        alert("No puede estar vacío");//lang.not_empty
        return false;
    }
    return true;
}
...
function check_pint(v)
{
    ...
    if(!regu.test(v))
    {
        alert("Debe ser un entero");//lang.only_int
        return false;
    }
    return true;
}
...
function check_max(v)
{
    ...
    if(parseInt(v) > parseInt(max))
    {
        ...
        alert("Ámbito de aplicación 0~"+max);//lang.small
        return false;
    }
    return true;
}
