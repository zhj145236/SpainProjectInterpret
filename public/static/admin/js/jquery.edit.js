...
function required(str, s_value, jqobj)
{
    if (str == '')
    {
        ...
        jqobj.remove();
        alert('Este elemento no puede estar vacío');
        return 0;
    }
    return 1;
}
...
function check_type(type, value, s_value, jqobj)
{
    if (type == 'number')
    {
        if (isNaN(value))
        {
            jqobj.prev('span').show().text(s_value);
            jqobj.remove();
            alert('Este elemento sólo puede ser numérico');
            return 0;
        }
    }
    if (type == 'int')
    {
        ...
        if (!regu.test(value))
        {
            jqobj.prev('span').show().text(s_value);
            jqobj.remove();
            alert('Este elemento sólo puede ser entero');
            return 0;
        }
    }
    if (type == 'pint')
    {
        ...
        if (!regu.test(value))
        {
            jqobj.prev('span').show().text(s_value);
            jqobj.remove();
            alert('Este elemento sólo puede ser un entero positivo');
            return 0;
        }
    }
    if (type == 'zint')
    ...
    if (type == 'discount')
    {
        ...
        if (!regu.test(value))
        {
            jqobj.prev('span').show().text(s_value);
            jqobj.remove();
            alert('Sólo puede ser un número entre 0.1 y 9.9');
            return 0;
        }
    }
    return 1;
...
function check_max(str, s_value, max, jqobj)
{
    if (parseInt(str) > parseInt(max))
    {
        jqobj.prev('span').show().text(s_value);
        jqobj.remove();
        alert('Este término debe ser inferior o igual a' + max);
        return 0;
    }
    return 1;
}
