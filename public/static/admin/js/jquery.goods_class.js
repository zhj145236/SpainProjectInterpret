            ...
            $.ajax({
                ...
                success: function(data) {
                    ...
                    for (var i = 0; i < data.length; i++) {
                        ...
                        //排序
                        src += " <span title='Clasificación de subcategorías editables' ajax_branch='goods_class_sort' datatype='number' fieldid='" + data[i].gc_id + "' fieldname='gc_sort' ds_type='inline_edit' class='editable tooltip'>" + data[i].gc_sort + "</span></td>";
                        //名称
                        ...
                        src += " <span title='Editar nombre de categoría inferior' required='1' fieldid='" + data[i].gc_id + "' ajax_branch='goods_class_name' fieldname='gc_name' ds_type='inline_edit' class='editable tooltip'>" + data[i].gc_name + "</span>";
                        //新增下级
                        if (data[i].deep < 3) {
                            src += "<a class='btn-add-nofloat marginleft' href='"+ADMINSITEURL+"/Goodsclass/goods_class_add.html?gc_parent_id=" + data[i].gc_id + "'><span>Añadir nivel inferior</span></a>";
                        }
                        src += "</td>";
                        //是否显示
                        ...
                        //操作
                        src += "<td class='w84'>";
                        src += "<a href='"+ADMINSITEURL+"/Goodsclass/goods_class_edit.html?gc_id=" + data[i].gc_id + "'>Editar</a>";
                        src += " | <a href=\"javascript:dsLayerConfirm('" + ADMINSITEURL + "/Goodsclass/goods_class_del/gc_id/" + data[i].gc_id + "','Está seguro de que desea borrar？'," + data[i].gc_id + ");\">Borrar</a>";
                        src += "</td>";
                        src += "</tr>";
                    }
                    //插入
