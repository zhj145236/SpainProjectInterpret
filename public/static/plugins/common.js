$(function() {
    ...
    if (controller == "Storelist") {
        ...
        $('#search ul.tab li span').eq(1).html('Productos');
        $('#search-form').attr("action", HOMESITEURL + "/Storelist/index.html");
    } else {
        $('#search ul.tab li span').eq(0).html('Productos');
...
function login_dialog(){
    CUR_DIALOG = ajax_form('login','Iniciar sesión',HOMESITEURL+'/Login/login.html?inajax=1',360,1);
}
        ...
        if (result == '0') {
            ...
            $.getJSON(url, {'fid': fav_id}, function(data) {
                if (data.done) {
                    ...
                    if (jstype == 'succ') {
                        $('[dstype="' + jsobj + '"]').each(function() {
                            $(this).html("Colección exitosa");
                        });
                    }
                    if (jstype == 'store') {
                        $('[ds_store="' + fav_id + '"]').each(function() {
                            $(this).before('<span class="goods-favorite" title="La tienda ha sido recogida"><i class="have">&nbsp;</i></span>');
                            $(this).remove();
                        });
                    }
                }
        ...
        if (result) {
            ...
            if (result.cart_goods_num > 0) {
                for (var i in result['list']) {
                    ...
                    html += '<dd class="handle"><a href="javascript:void(0);" onClick="drop_topcart_item(' + goods['cart_id'] + ',' + goods['goods_id'] + ');">Borrar</a></dd>';
                    html += "</dl>";
                }
                obj.find('.incart-goods').html(html);
                obj.find('.incart-goods-box').perfectScrollbar('destroy');
                obj.find('.incart-goods-box').perfectScrollbar({suppressScrollX: true});
                html = "Total<i>" + result.cart_goods_num + "</i>Especie Productos&nbsp;&nbsp;Importe total：<em>&yen;" + result.cart_all_price + "</em>";
                obj.find('.total-price').html(html);
                mini.find('.total-price').html('<p>Total<em class="goods-price" style="margin-left: 5px">' + result.cart_goods_num + '</em>Especie Productos</p><p>Todos Total：<em class="goods-price">&yen;' + result.cart_all_price + '</em></p>');
                ...
                html = "<div class='no-order'><span>No hay productos en su carrito de la compra por el momento！</span></div>";
                obj.find('.incart-goods').html(html);
...
function load_history_information(){
    $.getJSON(HOMESITEURL+'/Index/viewed_info.html', function(result){
        ...
        if (text_append == '') text_append = '<li class="no-goods">Bienes no disponibles</li>';;
	...
	$.fn.membershipCard = function(options){
		                ...
		                success: function(data) {
		                	if(data){
		                		...
		                		if(typeof connect !== 'undefined' && connect === 1 && data.follow != 2){
		                			...
		                			var text_html = 'Fuera de línea';
		                			if (typeof user_list[data.id] !== 'undefined' && user_list[data.id]['online'] > 0 ) {
		                				class_html = 'chat_online';
		                				text_html = 'En línea';
		                			}
		                			_info += '<a class="chat '+class_html+'" title="Haga clic aquí para enviarme un mensaje" href="JavaScript:chat('+data.id+');">'+text_html+'</a>';
		                		}
		                		if(data.qq != ''){
		                			_info += '<a target="_blank" href="http://wpa.qq.com/msgrd?v=3&uin='+data.qq+'&site=qq&menu=yes" title="QQ: '+data.qq+'"><img border="0" src="http://wpa.qq.com/pa?p=2:'+data.qq+':52" style=" vertical-align: middle;"/></a>';
		                		}
		                		if(data.ww != ''){
		                			_info += '<a target="_blank" href="http://amos.im.alisoft.com/msg.aw?v=2&amp;uid='+data.ww+'&site=cntaobao&s=1" ><img border="0" src="http://amos.im.alisoft.com/online.aw?v=2&uid='+data.ww+'&site=cntaobao&s=2" alt="Haga clic aquí para enviarme un mensaje" style=" vertical-align: middle;"/></a>';
		                		}
		                		if(_info == ''){
		                			_info = '--';
		                		}
		                		var _ul = $('<ul></ul>').append('<li>Ciudad：'+((data.areainfo != null)?data.areainfo:'--')+'</li>')
		                			.append('<li>Cumpleaños：'+((data.birthday != null)?data.birthday:'--')+'</li>')
		                			.append('<li>Contacto：'+_info+'</li>').appendTo('<dd class="info"></dd>').parent().appendTo(_dl);
		                		// ajax info
		                		if(data.url != ''){
		                		...
		                		if(data.follow != 2){
		                			...
		                			if(data.follow == 1){
		                				$('<div class="follow-handle" dstype="follow-handle'+data.id+'" data-param="{\'mid\':'+data.id+'}"></div>')
		                					.append('<a href="javascript:void(0);" >Atención</a>')
		                					.append('<a href="javascript:void(0);" dstype="nofollow">Cancelar Atención</a>').find('a[dstype="nofollow"]').click(function(){
		                						onfollow($(this));
		                					}).end().appendTo(_bottom);
		                			}else{
		                				$('<div class="follow-handle" dstype="follow-handle'+data.id+'" data-param="{\'mid\':'+data.id+'}"></div>')
		                					.append('<a href="javascript:void(0);" dstype="follow">Más Atención</a>').find('a[dstype="follow"]').click(function(){
		                						follow($(this));
		                					}).end().appendTo(_bottom);
		                			}
		                			$('<div class="send-msg"> <a href="'+HOMESITEURL+'/Membermessage/sendmsg.html?member_id='+data.id+'" target="_blank"><i></i>Carta en la estación</a> </div>').appendTo(_bottom);
		                		}
		...
		function follow(o){
			...
			$.getJSON(HOMESITEURL+'/Membersnsfriend/addfollow.html?callback=?&mid='+data_str.mid, function(data){
				if(data.code==10000){
					$('[dstype="follow-handle'+data_str.mid+'"]').html('<a href="javascript:void(0);" >Atención</a> <a href="javascript:void(0);" dstype="nofollow">Más Atención</a>').find('a[dstype="nofollow"]').click(function(){
						onfollow($(this));
					});
				}
			});
		...
		function onfollow(o){
			...
			$.getJSON(HOMESITEURL+'/Membersnsfriend/delfollow.html?callback=?&mid='+data_str.mid, function(data){
				if(data.code==10000){
					$('[dstype="follow-handle'+data_str.mid+'"]').html('<a href="javascript:void(0);" dstype="follow">Prestar más atención</a>').find('a[dstype="follow"]').click(function(){
						follow($(this));
					});
				}
			});
		...
		function shop_function(d){
			return $('<dd class="ajax-info">Crédito del comprador：'+((d.member_credit == 0)?'Falta de crédito':d.member_credit)+'</dd>');
		}
	};
})(jQuery);
    ...
    $.fn.ds_region = function(options) {
        ...
        this.each(function() {
            ...
            if ($inputArea.val() === "") {
                ...
                var $region_btn = $('<input type="button" class="input-btn" ' + settings.btn_style_html + ' value="Editar" />');
        ...
        function loadArea($area, area_id) {
            if ($area && ds_a[area_id].length > 0) {
                ...
                if (settings.tip_type && settings.last_text != "") {
                    ...
                    $area.append("<option value=''>-Por favor, elija-</option>")
                }
    ...
    $.getJSON(url, {'goods_id': goods_id, 'quantity': quantity}, function(data) {
        if (data != null) {
            if (data.state) {
                ...
                if(dir) {
                    layer.msg('Añadir carrito de la compra con éxito');
                }
            } else {
                layer.msg(data.msg);
            }
