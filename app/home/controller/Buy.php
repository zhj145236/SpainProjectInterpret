...
class Buy extends BaseMember {
    ...
    public function pay() {
        ...
        $condition[] = array('order_state','in', array_values(array(ORDER_STATE_NEW, ORDER_STATE_PAY, ORDER_STATE_PICKUP)));
        $order_list = $order_model->getOrderList($condition, '', 'order_id,order_state,payment_code,order_amount,rcb_amount,pd_amount,order_sn,trade_no');
        if (empty($order_list)) {
        ...
        foreach ($order_list as $key => $order_info) {
            ...
            $payment_tradeno = $order_info['trade_no'];
        ...
        View::assign('payment_tradeno', $payment_tradeno);
        ...
        if ($pay_amount_online > 0) {
            ...
            foreach ($order_list as &$order) {
                $condition = array();
                $condition[] = array('payment_code','=',$order['payment_code']);
                $condition[] = array('payment_platform','=','pc');
                $condition[] = array('payment_state','=','1');
                $payment_info = $payment_model->getPaymentInfo($condition);
                if(empty($payment_info)){
                    $this->error(lang('appropriate_payment_method'),'home/Memberorder/index');
                }
                $order['payment_state'] = $payment_info['payment_name'];
                $payment_info['payment_config'] = unserialize($payment_info['payment_config']);
                $order['payment_desc'] = $payment_info['payment_name'];
                if(!empty($payment_info['payment_config'])){
                    $order['payment_desc'] = $payment_info['payment_config']['bankname'] . ' ' . $payment_info['payment_config']['banksn'];
                }
