...
class Buy_1 {
    ...
    public function calcStoreFreight($freight_list) {
        if (!is_array($freight_list) || empty($freight_list))
            return;
        //免费和固定运费计算结果
            ...
            foreach ($nocalced_list as $store_id => $value) {
                if (is_array($value)) {
                    foreach ($value as $transport_id => $v) {
                        ...
                        $freight_total = $transport_model->calcTransport($transport_id, $v['count'], $v['weight']);
...
//                 if (empty($pd_payment_info)) return;
    ...
    public function getReciverAddr($address_info = array()) {
        if (intval($address_info['chain_id'])) {
            $reciver_info['phone'] = trim($address_info['chain_mobile'] . ($address_info['chain_telephony'] ? ',' . $address_info['chain_telephony'] : null), ',');
            $reciver_info['tel_phone'] = $address_info['chain_telephony'];
            $reciver_info['mob_phone'] = $address_info['chain_mobile'];
...
//            $inv['送票地址'] = $invoice_info['invoice_goto_addr'];
    ...
    private function _getGoodsOnlineInfo($goods_id, $quantity) {
        ...
        $new_array['goods_ispifa'] = $goods_info['goods_ispifa'];
        $new_array['startnum1'] = $goods_info['startnum1'];
        $new_array['endnum1'] = $goods_info['endnum1'];
        $new_array['pifaprice1'] = $goods_info['pifaprice1'];
        $new_array['startnum2'] = $goods_info['startnum2'];
        $new_array['endnum2'] = $goods_info['endnum2'];
        $new_array['pifaprice2'] = $goods_info['pifaprice2'];
        $new_array['startnum3'] = $goods_info['startnum3'];
        $new_array['endnum3'] = $goods_info['endnum3'];
        $new_array['pifaprice3'] = $goods_info['pifaprice3'];
        
        if($new_array['goods_ispifa']>0){
        	if($new_array['startnum1'] <= $new_array['goods_num'] && $new_array['goods_num'] <= $new_array['endnum1']){
        		$new_array['goods_price'] = $new_array['pifaprice1'];
        	}else if($new_array['startnum2'] <= $new_array['goods_num'] && $new_array['goods_num'] <= $new_array['endnum2']){
        		$new_array['goods_price'] = $new_array['pifaprice2'];
        	}else if($new_array['startnum3'] <= $new_array['goods_num'] && $new_array['goods_num'] <= $new_array['endnum3']){
        		$new_array['goods_price'] = $new_array['pifaprice3'];
        	}
        }
