...
class Evaluategoods extends BaseModel {
     ...
     * @return array
     */
    public function getMobileEvaluategoodsList($condition, $field = '*', $pagesize = 0, $limit = 0, $order = 'geval_id desc') {
        
        $list = Db::name('evaluategoods')->field($field)->where($condition)->page($pagesize,$limit)->order($order)->select()->toArray();
        return $list;
    }
