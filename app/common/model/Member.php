...
class Member extends BaseModel
{
    ...
    public function register($register_info)
    {
        ...
        if (is_array($check_member_name) && !empty($check_member_name)) {
            return array('error' => 'El nombre de usuario ya existe');
        }
        ...
        if ($insert_id) {
            ...
            return array('error' => 'Falló el registro');
        }
    }
    /**
    ...
    public function addMember($data)
    {
        ...
        try {
            ...
            // 添加默认相册
            $insert = array();
            $insert['ac_name'] = 'Comprar un espectáculo familiar';
            $insert['member_id'] = $insert_id;
            $insert['ac_des'] = 'álbum predeterminado del programa del comprador';
            $insert['ac_sort'] = 255;
    ...
    public function getOneMemberGrade($exppoints, $show_progress = false, $member_grade = array())
    {
        ...
        if (empty($member_grade)) {//如果会员等级设置为空
            $grade_arr['level'] = -1;
            $grade_arr['level_name'] = 'No hay rango';
            return $grade_arr;
        }
