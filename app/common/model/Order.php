...
class Order extends BaseModel {
     ...
     * @return Ambigous <multitype:boolean Ambigous <string, mixed> , unknown>
     */
    public function getMobileOrderList($condition, $field = '*', $pagesize = 0, $limit = 0, $order = 'order_id desc', $extend = array()) {

        $list = Db::name('order')->field($field)->where($condition)->page($pagesize,$limit)->order($order)->select()->toArray();
        ...
        foreach ($list as &$order) {
            ...
            if (!empty($extend)){
                //追加返回订单扩展表信息
                if (in_array('order_common', $extend)) {
                    $order_common_list = $this->getOrdercommonList(array(array('order_id','=', $order['order_id'])));
                    foreach ($order_common_list as &$value) {
                        $value['reciver_info'] = @unserialize($value['reciver_info']);
                        $value['invoice_info'] = @unserialize($value['invoice_info']);
                        $order['extend_order_common'][] = $value;
                    }
                }
                //追加返回买家信息
                if (in_array('member', $extend)) {
                ...
                if (in_array('order_goods', $extend)) {
                    //取商品列表
                    $order_goods_list = Db::name('ordergoods')->where('order_id', '=', $order['order_id'])->select()->toArray();

                    if (!empty($order_goods_list)) {
                        foreach ($order_goods_list as &$val) {
                            if(!empty($val['goods_image'])){
                                $val['goods_image'] = goods_cthumb($val['goods_image']);
                            }
                        }
                        $order['order_goods'] = $order_goods_list;
                    } else {
    ...
    public function getOrderOperateState($operate, $order_info) {
        ...
        switch ($operate) {
            ...
            case 'system_receive_pay':
                ...
                $state = $order_info['order_state'] == ORDER_STATE_NEW && $order_info['payment_code'] != 'predeposit' && !empty($order_info['trade_no']);
                break;
