...
class Predeposit extends AdminControl {
    ...
    public function pdrecharge_list() {
        ...
        if (input('param.confirmstate_search') != '') {
            $condition[]=array('pdr_status','=',input('param.confirmstate_search'));
        }
    ...
    public function recharge_info() {
        ...
        if (!request()->isPost()) {
            ...
            $upcondition = array();
            $upcondition[] = array('pdr_id','=',$id);
            $upcondition[] = array('pdr_sn','=',$info['pdr_sn']);
            $upcondition[] = array('pdr_status','=',0);
            $update = array();
            $update['pdr_status'] = input('post.pdr_status');
            $update['pdr_admin_remark'] = trim(input('post.lg_desc'));
            $update['pdr_confirmtime'] = TIMESTAMP;
            $update['pdr_admin'] = $this->admin_info['admin_name'];
            if($update['pdr_status'] == 1){
                //上传文件保存路径
                if (!empty($_FILES['pdr_confirmimgs']['name'])) {
                    $file_name = "rechargeid_" . $id . "_" . date('YmdHis') . ".jpg";

                    $res = ds_upload_pic(ATTACH_RECHARGE, 'pdr_confirmimgs', $file_name);
                    if($res['code']){
                        $file_name=$res['data']['file_name'];
                        $update['pdr_serialnumber'] = trim(input('post.pdr_serialnumber'));
                        $update['pdr_amount'] = abs(floatval(input('post.pdr_amount')));
                        $update['pdr_confirmimgs'] = $file_name;
                    }else{
                        $this->error($res['msg']);
                    }
                } else {
                    $this->error(lang('admin_predeposit_recharge_dzimgs_null'));
                }
            }else if($update['pdr_status'] == '-1' && empty($update['pdr_admin_remark'])){
                $this->error(lang('admin_predeposit_adminremark_null'));
            }
            ...
            try {
                //更改充值状态
                $state = $predeposit_model->editPdRecharge($update, $upcondition);
                if (!$state) {
                    throw Exception(lang('admin_predeposit_recharge_edit_fail'));
                }
                if($update['pdr_status'] == 1){
                    //变更会员预存款
                    $data = array();
    ...
    public function export_step1() {
        ...
        if (!is_numeric(input('param.curpage'))) {
            ...
            if ($count > self::EXPORT_SIZE) { //显示下载链接
                ...
                $rechargeconfirmstate = array(0 => lang('admin_predeposit_rechargewaitconfirm'), 1 => lang('admin_predeposit_rechargeconfirmsuccess'), 2 => lang('admin_predeposit_rechargeconfirmfail'));
                foreach ($data as $k => $v) {
                    $data[$k]['pdr_status'] = $rechargeconfirmstate[($v['pdr_status']<0?2:$v['pdr_status'])];
                }
    ...
    private function createExcel($data = array()) {
        ...
        $excel_data[0][] = array('styleid' => 's_title', 'data' => lang('exp_yc_zzbankname'));
        $excel_data[0][] = array('styleid' => 's_title', 'data' => lang('exp_yc_zzbank'));
        $excel_data[0][] = array('styleid' => 's_title', 'data' => lang('exp_yc_zzimgs'));
        $excel_data[0][] = array('styleid' => 's_title', 'data' => lang('exp_yc_ctime'));
        $excel_data[0][] = array('styleid' => 's_title', 'data' => lang('exp_yc_banksn'));
        $excel_data[0][] = array('styleid' => 's_title', 'data' => lang('exp_yc_dzimgs'));
        $excel_data[0][] = array('styleid' => 's_title', 'data' => lang('exp_yc_cftime'));
        $excel_data[0][] = array('styleid' => 's_title', 'data' => lang('exp_yc_money'));
        $excel_data[0][] = array('styleid' => 's_title', 'data' => lang('exp_yc_confirmstate'));
        $excel_data[0][] = array('styleid' => 's_title', 'data' => lang('exp_yc_memberid'));
        foreach ((array) $data as $k => $v) {
            $tmp = array();
            $tmp[] = array('data' => $v['pdr_sn']);
            $tmp[] = array('data' => $v['pdr_member_name']);
            $tmp[] = array('data' => $v['pdr_paymentbankname']);
            $tmp[] = array('data' => $v['pdr_paymentbank']);
            if(!empty($v['pdr_paymentimgs'])){
                $v['pdr_paymentimgs'] = ds_get_pic(ATTACH_RECHARGE,$v['pdr_paymentimgs']);
            }
            $tmp[] = array('data' => $v['pdr_paymentimgs']);
            $tmp[] = array('data' => date('Y-m-d H:i:s', $v['pdr_addtime']));
            $tmp[] = array('data' => $v['pdr_serialnumber']);
            if(!empty($v['pdr_confirmimgs'])){
                $v['pdr_confirmimgs'] = ds_get_pic(ATTACH_RECHARGE,$v['pdr_confirmimgs']);
            }
            $tmp[] = array('data' => $v['pdr_confirmimgs']);
            if (intval($v['pdr_confirmtime'])) {
                if (date('His', $v['pdr_confirmtime']) == 0) {
                    $tmp[] = array('data' => date('Y-m-d', $v['pdr_confirmtime']));
                } else {
                    $tmp[] = array('data' => date('Y-m-d H:i:s', $v['pdr_confirmtime']));
                }
            } else {
                $tmp[] = array('data' => '');
            }
            $tmp[] = array('format' => 'Number', 'data' => ds_price_format($v['pdr_amount']));
            $tmp[] = array('data' => $v['pdr_status']);
