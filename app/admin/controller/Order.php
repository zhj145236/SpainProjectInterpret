...
class Order extends AdminControl {
    ...
    private function _order_receive_pay($post) {
        ...
        $order_list = $order_model->getOrderList($condition, 0, 'order_id,order_state,payment_code,order_amount,rcb_amount,pd_amount,order_sn,trade_no');
        if (empty($order_list)) {
            return ds_callback(false, lang('no_right_operate'));
        }
        //重新计算在线支付金额
        ...
        foreach($order_list as $order_info){
            ...
            $buyer_tradeno = $order_info['trade_no'];
        }


        if (!request()->isPost()) {
            View::assign('buyer_tradeno', $buyer_tradeno);
