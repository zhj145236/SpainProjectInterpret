...
class Refund extends AdminControl
{
        ...
        if (request()->isPost()) {
            ...
            if ($state) {
                ...
                $param['param'] = array_merge($param['ali_param'],array(
                    'refund_url' => (string)url('home/memberrefund/view', array('refund_id' => $refund['refund_id'])),
                ));
                //微信模板消息
    ...
    public function export_step1() {
        ...
        if (!is_numeric(input('param.curpage'))) {
            ...
            $data = $refundreturn_model->getRefundList(array_merge($condition,array(array('refund_type','=',1))), '', '*', 'refund_id desc', "{$limit1},{$limit2}");
            $this->createExcel($data);
        }
    }
